...
func (c *Collector) Start() {
	...
	if args.Output != "" {
		...
		// Use a buffered writer and strconv to avoid fmt.Sprintf allocations
		w := bufio.NewWriter(f)
		_, err = w.WriteString("url,latency,status_code,error\n")
		if err != nil {
			panic(err)
		}
		for _, sample := range c.Samples {
			// write url
			_, err = w.WriteString(sample.Url)
			if err != nil {
				panic(err)
			}
			_ = w.WriteByte(',')
			// write latency
			_, err = w.WriteString(strconv.FormatInt(sample.Latency, 10))
			if err != nil {
				panic(err)
			}
			_ = w.WriteByte(',')
			// write status code
			_, err = w.WriteString(strconv.Itoa(sample.StatusCode))
			if err != nil {
				panic(err)
			}
			_ = w.WriteByte(',')
			// write error string
			_, err = w.WriteString(sample.ErrStr)
			if err != nil {
				panic(err)
			}
			_ = w.WriteByte('\n')
